...
import { ZH_TW_L1_OVERRIDES } from './wordDefinitionsZhTW_L1_overrides.js';
import { WORD_DEFINITIONS_ZH_TW_L1 } from './wordDefinitionsZhTW_L1.js';
import { ZH_TW_L2_OVERRIDES } from './wordDefinitionsZhTW_L2_overrides.js';
...
function expandDefinition(item, wordLevel = 1) {
  const [
    id,
    synsetId,
    partOfSpeech,
...
export function normalizeChineseMeaning(meaning) {
  if (!meaning) return '';
  return meaning
    .trim()
    .replace(/\s+/g, ' ')
...
export function deduplicateLearnerDefinitions(definitions) {
  ...
  definitions.forEach((def) => {
    ...
    if (!groupByKey.has(key)) {
      const displayItem = {
        ...
        partOfSpeechLabel: posKey,
        primarySenseId: def.id,
        sourceSenseIds: [def.id],
        englishDefinition: def.englishDefinition || def.definition,
        sourceDefinitions: [def],
      };

      groupByKey.set(key, displayItem);
      groups.push(displayItem);
    } else {
      const existing = groupByKey.get(key);
      existing.sourceSenseIds.push(def.id);
      existing.sourceDefinitions.push(def);
    }
  });
...
export function getWordDefinitions(wordId) {
  const entry = WORD_DEFINITIONS_BY_ID[String(wordId)];

  if (!entry) {
    return null;
  }

  const wordLevel = WORD_LEVEL_BY_ID.get(String(wordId)) || 1;

  const rawPrimaryDefinitions = (entry.p ?? []).map((item) => expandDefinition(item, wordLevel));
  const rawSecondaryDefinitions = (entry.s ?? []).map((item) => expandDefinition(item, wordLevel));

  const primaryDefinitions = deduplicateLearnerDefinitions(rawPrimaryDefinitions);
  const secondaryDefinitions = deduplicateLearnerDefinitions(rawSecondaryDefinitions);

  return {
    primaryDefinitions,
    secondaryDefinitions,
    rawPrimaryDefinitions,
    rawSecondaryDefinitions,
    definitions: primaryDefinitions.concat(secondaryDefinitions),
    rawDefinitions: rawPrimaryDefinitions.concat(rawSecondaryDefinitions),
  };
}
`;
